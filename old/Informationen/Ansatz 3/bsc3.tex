...
\newtheorem{Satz}{Sentence}[section]
...
and define the domain of $A$ as follows
	...
	\end{align*}
	\pdfcomment{is this really the domain? I dropped the condition that it is continuous in the points since $H^{2} \Subset C^{1}$ or am I wrong? Or is it H^2(x_1-0.5, x_i) cap H^2(x_i, x_i+0.5)}
	...
	Next, we show that $R_{\mu}$ is well-defined, which means that for all $u \in \mathcal{D}(A)$ there exists a unique solution. Lets assume that $f$, as the righthand-side of the given differential equation, is a bounded linear functional. Lax-Milgram's theorem\footnote{formulation and prove in appendix A} would then guarantee the existence and uniqueness of a solution if $\mathcal{D}(A)$ is a Hilbert space\pdfcomment{Is that really the requirement} and if the left-hand side as bilinear 
	\begin{align*}
...
%for a $u \in H^{1}_{k} \coloneqq \big\{ u \in H^{1}(\Omega) : u(\nicefrac{1}{2}) = e^{ik} u(-\nicefrac{1}{2}) \big\}$ and for all $\varphi \in H^{1}_{k}$
%
%\begin{align*}
%	dom A_{0} = \Big\{ u & \in H^{2}(-\nicefrac{1}{2}, 0) \cap H^{2}(0, \nicefrac{1}{2}), u(-0) = u(+0), \\
%		& \quad u'(-0) - u'(+0) + c u(0) = 0, u(\nicefrac{1}{2}) = e^{ik} u(-\nicefrac{1}{2}), u'(\nicefrac{1}{2}) = e^{ik} u'(-\nicefrac{1}{2}) \Big\} 
%\end{align*}
%~\
%
...
%	Now choosing a specific $v$ such that $\supp v = (-\nicefrac{1}{2}, x_{0})$ alters $(*)$ to
%	\[ \int_{-\nicefrac{1}{2}}^{x_{0}} u' v' dx = \int_{-\nicefrac{1}{2}}^{x_{0}} A u \overline{v} \iff \int_{-\nicefrac{1}{2}}^{x_{0}} u \overline{v}'' dx = \int_{-\nicefrac{1}{2}}^{x_{0}} - A u \overline{v} dx \]
%
%	$\Rightarrow u'' = - A u \in L^{2}$ on $(-\nicefrac{1}{2}, x_{0})$ and an analogously on $(x_{0}, \nicefrac{1}{2})$. One can therefore fix
%	\[ dom A \subset \{ u \in H^{2}(-\nicefrac{1}{2}, x_{0}), u \in H^{2}(x_{0}, \nicefrac{1}{2}) \} \]
%
%	Next, integrating $(*)$ on both sides of $x_{0}$ by parts yields 
%	\[ -\left( \int_{-\nicefrac{1}{2}}^{x_{0}} + \int_{x_{0}}^{\nicefrac{1}{2}}\right) u'' \overline{v} + \big( u'(x_{0}-0) v(x_{0} - 0) - u'(x_{0} + 0)v(x_{0}+0) \big) + c u(x_{0})\overline{v}(x_{0}) \]
%	\[  = - \int_{-\nicefrac{1}{2}}^{x_{0}} u'' v - \int_{x_{0}}^{\nicefrac{1}{2}} u'' v \]
%
...
%	\[ \iff - \int_{-\nicefrac{1}{2}}^{x_{0}} u'' v - \int_{x_{0}}^{\nicefrac{1}{2}} u'' v = \int_{\Omega} u' v' + c u(x_{0}) v(x_{0}) \]
%	Partial integration with $\supp v = (-\nicefrac{1}{2}, \nicefrac{1}{2})$ yields
%	\[ \int_{-\nicefrac{1}{2}}^{x_{0}} u'v' + \int_{x_{0}}^{\nicefrac{1}{2}} u'v' -u'(x_{0}-0) v(x_{0})  + u'(x_{0}+0) v(x_{0}) = \int_{\Omega} u' v' + c u(x_{0}) v(x_{0}) \]
