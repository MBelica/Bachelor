...
\end{equation}
on the whole of $\R$, where $f$ is a function modelling an external force, $\delta$ denotes the Dirac delta distribution and $x_{i}$ are periodically distributed points on $\R$. $\Omega_{k}$ will hereafter indentify the periodicity cell containing delta point $x_{k}$ and let w.o.l.g. $x_{0} = 0$ and $|\Omega_{i}| = 1 ~\forall i \in \Z$.  Henceforth, consider	for a $\mu \in \R$ small enough the problem 
\begin{equation}
