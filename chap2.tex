...
Furthermore, $A$ is self-adjoint which will will be later impoartant.\footnote{Here HAS to be some more text but I don't know what}

\begin{theorem}
	$A$ is a self-adjoint operator
		
	\begin{proof} % 	TODO prove that $R_{\mu}$ is symmetric and with that $A$ self-adjoint and that
		We focus first on $R_{\mu}(A)^{-1} = (A - \mu I)$, which is symmetric, as $\forall v \in H^{1}$:
			\begin{align*}
		...
		Now as $\mathcal{D}(R_{\mu}) = L^{2}(\R)$ and $\mathcal{R}(R_{\mu}) = \mathcal{D}(R_{\mu}^{-1})$, we want to show that for each $f, g \in L^{2}(\R)$
		
		\[ \gamma \coloneqq \langle R_{\mu} f, g \rangle - \langle f, R_{\mu} g \rangle = 0 \]
		% todo todododod
		Now, choose $u, v \in \mathcal{D}(A)$ such that $R_{\mu}f = u, R_{\mu} g = v$. Using this fact in combination with \eqref{weak-formulation} for those two $u, v$ one gets for all $\varphi, \psi \in H^{1}$
		...
		As it has to hold for all $\varphi, \psi \in H^{1}_{k}$ the special choice of $\varphi = v$ and $\psi = u$ yields $\gamma = 0$ and $R_{\mu}$ is therefore symmetric. All in all we can use this to show that $R_{\mu}$ is self-adjoint, as we get for an arbitrary $v^{*} \in \mathcal{D}(R_{\mu}^{-1})$ there exists a $v \in \mathcal{R}(R_{\mu}^{-1}) = \mathcal{D}(R_{\mu})$: \footnote{I haven't understood this proof yet. As R is symmetric D(R*) >= D(R) we therefore have to show D(R*) <= D(R) to show the self-adjointness...}
