\chapter{The one-dimensional Schrödinger operator} \label{chap2}
...
\end{equation}
on the whole of $\R$, where $\delta_{x_{i}}$ denotes the Dirac delta distribution supported at the point $x_{i}$. The second term repre $\Omega_{k}$ will hereafter identify the periodicity cell containing point $x_{k}$ and w.l.o.g. let $x_{0} = 0$ and $|\Omega_{i}| = 1$ for all $i \in \Z$.
~\\  
...
\begin{equation} % todo check formular
...
\begin{proof} % todo do a better proof
...
\begin{theorem} \label{2.3:thm-ASelfAdjoint}
	...
	\begin{proof}
		We proceed by proving first that $R_{\mu}^{-1}$ is self-adjoint. As we already know that $R_{\mu}^{-1}$ are symmetric, showing that $R_{\mu}^{-1}$ is self-adjoint is equivalent to showing that if $v \in \mathcal{D}({R_{\mu}^{-1}}^{*})$ and $v^{*} \in L^{2}(\R)$ are such that % todo erwähnen dass  D((R_\mu^-1)*) \subset D(R_{\mu})? bzw anders herum bin mir nicht sicher
		\begin{align}
