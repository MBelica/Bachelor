...
~\\  
In general, one cannot say, given $f \in L^{2}(\R)$, in which sense a solution to the problem 
	\begin{equation}
...
\begin{remark}
	Since $C_{0}^{\infty}(\R)$ is dense in $H^{1}(\R)$ with respect to the norm in $H^{1}(\R)$, \eqref{weak-formulation-of-A} holds also for all $v \in H^{1}(\R)$. % todo INTRODUCTION
\end{remark}
