...
\begin{remark}
	This definition is independent of the chosen sequence since	 % todo closure H1 dicht
\end{remark}
...
\begin{figure}[!ht] \centering
	\resizebox{.35\linewidth}{!}{
	  ...
	  \draw (-1.2,0.125) node[anchor=north west] {$Y_1^-$};
	  \draw (1.05,0.125) node[anchor=north west] {$Y_1^+$};
	  \draw (-0.125,-1.05) node[anchor=north west] {$Y_2^-$};
	  \draw (-0.125, 1.25) node[anchor=north west] {$Y_2^+$};
	  \end{tikzpicture}
	}
...
\begin{remark} 
	...
	Both terms on the right-hand side can then be finitely estimated by the Trace Theorem \cite[page 258]{evans1998partial} and Poincaré inequality for some $h > 0$ through % todo diese Abschötzung
...
\begin{figure}[!ht] \centering
	\resizebox{.40\linewidth}{!}{
	  \begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=5.0cm,y=5.0cm]
		...
		\draw (0.178,0.358) node[anchor=north west] {$S$};
		\draw (-0.852,-0.546) node[anchor=north west] {$Y$};
		\draw [->] (0.1416,0.469) -- (0.2034,0.7943);
		\draw (0.204,0.769) node[anchor=north west] {$\eta$};
		\draw (-0.015,-0.240) node[anchor=north west] {$-$};
		\draw (0.151,-0.493) node[anchor=north west] {$+$};
	  \end{tikzpicture}
	}
...
\begin{proof}
 	In Section \ref{sec:3.2} we used particular functions $v \in C^{\infty}(\R)$ to prove equivalent properties of $\mathcal{D}(A)$. By using the same approach with equivalent functions for the multi-dimensional case ing in \eqref{eq:7.2-md-weak-formulation} asserted follows.
\end{proof}
...
We restrict this multi-dimensional problem to a corresponding fundamental domain of periodicity, let this for simplicity be $Y$. Let us consider the problem to find 
	\[ u \in H^{1}_{k, n} \coloneqq \left\{ w \in H^{1}(Y) \colon w \big|_{Y_{j}^{+}} = w \big|_{Y_{j}^{-}} e^{i k_{j}} \text{ for } k \in [-\pi, \pi]^{2}, j = 1,2 \right\} \] % todo derivates?!
such that
	\begin{equation}
