...
We are interested in the periodic spectral problem of the operator $A^{n}$. Therefore, we will again relate the spectrum of the operator $A^{n}$ on the whole of $\R^{n}$ via the Floquet transform to a family of eigenvalue problems on the periodicity cell. However, first, we need to show the self-adjointness of the operator $A^{n}$.
...
The periodicity cell we will now focus on can be chosen arbitrary, let this for simplicity be $Y$. For brevity, let us define for $k \in \overline{B^{n}}$ 
