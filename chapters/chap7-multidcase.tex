...
such that $R_{\mu}^{n}$ is the resolvent of $A^{n}$. Therefore, we can characterise $\mathcal{D}(A^{n})$ by employing the same methods as used in Section \ref{sec:3.2}.
...
We are interested in the periodic spectral problem of the operator $A^{n}$. Therefore, we will again relate the spectrum of the operator $A^{n}$ on the whole of $\R^{n}$ via the Floquet transform to a family of eigenvalue problems on the periodicity cell. First, we need to show the self-adjointness of the operator $A^{n}$.
...
\begin{proof}
	The operators $R_{\mu}^{n}$ and $\left(R_{\mu}^{n}\right)^{-1}$ are symmetric. The fact that for any $u \in \mathcal{D}\left(\left(R_{\mu}^{n}\right)^{-1}\right)$ there exists $f \in L^{2}(\R^{n})$ such that $u = R_{\mu}^{n} f$ and that $R_{\mu}^{n}$ is defined on the whole of $L^{2}(\R^{n})$ then yields the self-adjointness of $A^{n}$ similarly to Section \ref{sec:3.3}.
\end{proof}
...
\begin{theorem}[Characterisation of $\mathcal{D}(A^{n}_{k})$] Let $Y \coloneqq \R^{n} \setminus \overline{\bigcup_{j \in \Z^{n}} D_{j}}$. We can characterise the solution $u \in \mathcal{D}(A^{n}_{k}) \subseteq H^{1}_{k, n}$ from \eqref{eq:7.2-md-weak-formulation}, namely that for all $j \in \Z^{n}$ it holds:
	\begin{enumerate}[label=\alph*\upshape)]
...
\begin{proof}
 	The first three properties follow as in Theorem \ref{thrm:7.2}. The quasi-periodic condition on the function $u \in \mathcal{D}(A^{n}_{k})$ follows from the boundary condition in $H^{1}_{k, n}$. Additionally, the quasi-periodic boundary conditions on $H^{1}_{k,n}$ require a solution $u \in H^{1}_{k, n}$ to satisfy furthermore quasi-periodic conditions for their weak derivative
