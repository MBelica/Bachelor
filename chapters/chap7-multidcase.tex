...
\begin{remark}
	This definition is independent of the chosen sequence since	 % todo X closure H1 dicht
\end{remark}
...
We restrict this multi-dimensional problem to a corresponding fundamental domain of periodicity, let this for simplicity be $Y$. Let us consider the problem to find 
	\[ u \in H^{1}_{k, n} \coloneqq \left\{ w \in H^{1}(Y) \colon w \big|_{Y_{j}^{+}} = w \big|_{Y_{j}^{-}} e^{i k_{j}} \text{ for } k \in [-\pi, \pi]^{2}, j = 1,2 \right\} \] % todo X derivates?!
such that
	\begin{equation}
