\chapter{The Floquet transformation and the Bloch waves}

In chapter \ref{chap4} we are going to show that the spectrum of the operator $A$ can be constructed from the eigenvalue sequences $(\lambda_{s}(k))_{s \in \N}$ introduced above by varying $k$ over the Brillouin zone $B$. For this purpose we will need two results involving the Floquet transformation to be able to move the problem from $L^{2}(\R)$ to $L^{2}(\Omega \times B)$ whereas $\Omega \times B$ is by assumption compact. For the sake of completeness, we include here the proofs of both theorems, as given in \cite[Chap. 3.4, 3.5]{Plum10}.
