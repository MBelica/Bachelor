...
Due to the fact that convergence in $H^{1}_{k}$ implies the convergence on the trace of $\Omega$, $H^{1}_{k}$ is a closed subspace of $H^{1}(\R)$, and one can therefore apply the same arguments as above to show that $R_{\mu, k} \colon L^{2}(\Omega) \rightarrow H^{1}_{k}, f \mapsto u$ is well-defined and define in return % todo Martrin: ref to above and trace convergence?
...
\begin{theorem} \label{3.1:thm-Rmuk.isCompact} % todo Martin/Markus so viele Kommentare
	...
	Hence, there are finitely many step functions such that for all $f \in L^{2}(\Omega)$ one of those step functions, let's call it $g \in L^{2}(\Omega)$, with function value $c_{k}$ on interval $I_{k}$ for all $k \in \{ 1, \dotsc, n_{\epsilon} \}$ sucht that
		...
		\end{align*}		 
	In conclusion this means that $B_{H^{1}_{k}}$ is totally bounded in $L^{2}(\Omega)$ and with the closure of $H^{1}_{k}$ this yields the compact embedding of $H^{1}_{k}$ in $L^{2}(\Omega)$. Thus, $R_{\mu, k}$ is compact. % todo Martin/Martin closure Bweis
	\end{proof}	
\end{theorem}		

\section{The Spectrum of the restricted operator \texorpdfstring{$A_{k}$}{Ak}}	% todo Markus  rewrite this a bit it isnt actually in H^1_k hmm
