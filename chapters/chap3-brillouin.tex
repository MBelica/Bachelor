...
In the next chapter we shall see that the spectrum of the operator $A$ can be constructed through the eigenvalue sequences $(\lambda_{s}(k))_{s \in \N}$ by varying $k$ over the Brillouin zone $B$. For that we need two results involving the Floquet transformation, an operator mapping from $L^{2}(\R)$ to $L^{2}(\Omega \times B)$ as $\Omega \times B$ is compact. \\
