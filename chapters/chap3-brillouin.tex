...
% todo Martin der Absatz?
Let $\Omega$ be the fundamental domain of periodicity associated with \eqref{the-operator-A-formally}, for simplicity let $\Omega \coloneqq \Omega_{0}$ and thus $x_{0} = 0$ being the delta point contained in $\Omega$. As commonly used in the literature the reciprocal lattice for $\Omega$ is $[-\pi, \pi]$, the so called one-dimensional Brillouin zone $B$. For fixed $k \in \overline{B}$, now consider the operator $A_{k}$ on $\Omega$ formally denoted by the operation % todo Markus über diese Absatz reden
...
Due to the fact that convergence in $H^{1}_{k}$ implies the convergence on the trace of $\Omega$, $H^{1}_{k}$ is a closed subspace of $H^{1}(\R)$, and one can therefore apply the same arguments as above to show that $R_{\mu, k} \colon L^{2}(\Omega) \rightarrow H^{1}_{k}, f \mapsto u$ is well-defined and define in return % todo Martrin: ref to above and trance convergence?
...
\begin{theorem} \label{3.1:thm-Rmuk.isCompact} % todo Markus so viele Kommentare
	...
	Which means that $(u_{j})_{j \geq 1}$ is bounded in $H^{1}(\Omega)$. As $H^1(\Omega) \subseteq C(\Omega)$ we can further estimate
		\begin{equation}
	...
	Hence, there are finitely many step functions such that for all $f \in L^{2}(\Omega)$ one of those step functions, let's call it $g \in L^{2}(\Omega)$, with function value $c_{k}$ on interval $I_{k}$ for all $k \in \{ 1, \dotsc, n_{\epsilon} \}$ sucht that
		...
		\end{align*}		 
	In conclusion this means that $B_{H^{1}_{k}}$ is totally bounded in $L^{2}(\Omega)$ and with the closure of $H^{1}_{k}$ this yields the compact embedding of $H^{1}_{k}$ in $L^{2}(\Omega)$. Thus, $R_{\mu, k}$ is compact. % todo Martin cloure
	\end{proof}	
\end{theorem}		

\section{The Spectrum of the restricted operator \texorpdfstring{$A_{k}$}{Ak}}	% todo  Martin rewrite this a bit
