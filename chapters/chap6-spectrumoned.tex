...
\begin{theorem} \label{4.1:thm-MainResult.FirstInclusion}
	...
	\begin{proof}
		...
		\end{equation} 
		We regard $\psi_{s}(\cdot, k)$ as extended to the whole of $\R$ by the boundary condition \eqref{quasi-periodic-condition}, whence, due to the periodic structure of $A$, $\psi_{s}$ satisfies % todo show that it \psi is in dom A
...
\end{figure}		
	 	...
	 	As $u_{l} \in \mathcal{D}(A)$, we see that % todo proof domain
		\begin{align}
