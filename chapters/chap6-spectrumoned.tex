...
\begin{theorem} \label{4.1:thm-MainResult.FirstInclusion}
	...
	\begin{proof}
		...
		we are able to define, for each $l \in \N$,
	 	...
	 	for an illustration see Figures \ref{fig:6.1}, \ref{fig:6.2} and \ref{fig:6.3} where a schematic of the real-part is depicted, the complex-part goes analogously. As $\psi_{s} \in \mathcal{D}(A)$ we know $\kappa_{l} \in \mathcal{D}(A)$, hence, we see that
		\begin{align}
...
\begin{figure}
 \begin{tikzpicture}
  \begin{axis}[ width=6in, height=2.25in, at={(1.011in,2.39in)}, scale only axis, xmin=-2.5, xmax=2.5, ymin=-150, ymax=175, ticks=none, axis x line=middle,
             axis y line=middle, yticklabels={,,}, xticklabels={,,} after end axis/.code={ \draw[red,->] (axis cs:0,0) -- (axis cs:0.5,0); }]
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ -2 -10 \\ -2 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ -1.5 -10 \\ -1.5 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ -1 -10 \\ -1 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ -0.5 -10 \\ -0.5 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ 0 -10 \\ 0 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ 0.5 -10 \\ 0.5 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ 1 -10 \\ 1 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ 1.5 -10 \\ 1.5 10 \\ };
	\addplot [color=black,solid,forget plot] table[row sep=crcr]{ 2 -10 \\ 2 10 \\ };
