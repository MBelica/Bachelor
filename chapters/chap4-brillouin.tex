...
\begin{definition} 
	...
	\begin{equation} 
		H^{1}_{k} \coloneqq \left\{ \psi \in H^{1}(\Omega): ~ \psi\left(\frac{1}{2}\right) = e^{ik} \psi\left(-\frac{1}{2}\right), ~ \psi'\left(\frac{1}{2}\right) = e^{ik} \psi'\left(-\frac{1}{2}\right) \right\}. \label{quasi-periodic-condition} % todo gehört die Ableitung hier rein oder erst in die Domain. Wenn die reingehören, dann muss ich im Beweis auch zeigen, dass auch das abgeschlossen ist.
	\end{equation}
...
We already know that $\mathcal{D}(A_{k}) \subseteq H^{1}_{k}$. Therefore, proceeding as in Section \ref{sec:3.2}, by choosing the same test functions $v \in H^{1}_{k}$ in \eqref{weak-formulation-to-the-restricted}, we are able to show that
\begin{align*}
