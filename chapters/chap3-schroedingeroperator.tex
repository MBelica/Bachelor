...
\begin{theorem} \label{2.3:thm-ASelfAdjoint}
	...
	\begin{proof}
		begin the proof of this theorem by proving that $R_{\mu}^{-1}$ is self-adjoint. As we already know that $R_{\mu}^{-1}$ is symmetric, showing that $R_{\mu}^{-1}$ is self-adjoint is equivalent to showing for $u \in \mathcal{D}(R_{\mu}^{-1})$ that if $v \in \mathcal{D}({R_{\mu}^{-1}}^{*})$ and $v^{*} \in L^{2}(\R)$ are such that
		\begin{align}
