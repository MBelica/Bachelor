...
\begin{theorem} \label{2.1:thm-LaxMilgram}
	...
	\begin{proof} ~\\
		...
		$b)$ Let $u \in H^{1}(\R)$. For the coercivity, we note first that for the given sesquilinear form $B[u, u] \in \R$ holds. Assuming $\rho \geq 0$ yields for $\mu \leq																												 -1$ that
		\begin{align*}
...
\begin{theorem} \label{rmuinj}
	...
	\begin{proof}
		...
		\[ \| R_{\mu} f \|_{L^{2}(\R)}^{2} \leq \int_{\R} |u'(x)|^{2} dx + \rho \sum_{i \in \Z} |u(x_{i})|^{2} - \mu \int_{\R} |u(x)|^{2} dx \leq \| f \|_{L^{2}(\R)} \| u \|_{L^{2}(\R)}, \]	% todo formel
...
\begin{remark}
	$R_{\mu}$ is hence the resolvent function of $A$.
\end{remark}
...
Using the fact that $R_{\mu}$ and $R_{\mu}^{-1}$ are symmetric operators we can now prove the main statement of this section. Since every self-adjoint operator has an entirely real spectrum, this theorem yields furthermore our first result about the spectrum of $A$, for a proof see Theorem \ref{spectrul-sa-real}. % todo further vs furthermore

\begin{theorem} \label{2.3:thm-ASelfAdjoint}
	$A$ is a self-adjoint operator.
		
	\begin{proof}
		We begin this theorem by proving that $R_{\mu}^{-1}$ is self-adjoint. As we already know that $R_{\mu}^{-1}$ is symmetric, showing that $R_{\mu}^{-1}$ is self-adjoint is equivalent to showing for $u \in \mathcal{D}(R_{\mu}^{-1})$ that if $v \in \mathcal{D}({R_{\mu}^{-1}}^{*})$ and $v^{*} \in L^{2}(\R)$ are such that
		\begin{align}
		...
		By Theorem \ref{rmuinj}, in \eqref{*-condition} exists $f \in L^{2}(\R)$ such that $u = R_{\mu} f$; using use the fact that $R_{\mu}$ is symmetric and defined on the whole of $L^{2}(\R)$ yields
