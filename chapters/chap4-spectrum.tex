...
Now, in the \ref{chap4}th chapter will now prove the main result for the one-dimensional case stating that for the operator $A$ it holds that
	\begin{equation}
		\sigma(A) = \bigcup_{s \in \N} I_{s}, \label{MainResult}
	\end{equation}
where $I_{s} \coloneqq \{ \lambda_{s}(k) : k \in \overline{B} \} ~(s \in \N)$. 
~\\ ~\\
To show this equality we first need to characterise the dependence of $I_{s}$ from $k$, more specifically that $\lambda_{s}(k)$ is continuous in $k$ and hence $I_{s}$ a compact interval.
	...
	\resizebox{.55\linewidth}{!}{
		\begin{tikzpicture}
		  \begin{axis}[
			...
			\addplot[color=blue,domain=-2.5:-0.5, smooth] {0};
			\addplot[color=blue,domain=-0.5:-0.25, smooth] {1/(1+(1/(2.2+4*x)^84))};
			\addplot[mark=none, domain=-0.25:0.25, blue, samples=2] {1};
			\addplot[color=blue,domain=0.25:0.5, smooth] {(1/(1+(1/(2.2-4*x)^84))))};
			\addplot[color=blue,domain=0.5:2.5, smooth] {0};
		  \end{axis}
		\end{tikzpicture}
	}
