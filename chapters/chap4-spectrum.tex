...
~\\ ~\\
To prove this equality we first need to characterise the dependence of $I_{s}$ from $k$, more specifically that $\lambda_{s}(k)$ is continuous in $k$ and hence $I_{s}$ being for each $k$ a compact interval.
	...
	\resizebox{.55\linewidth}{!}{
		\begin{tikzpicture}
		  \begin{axis}[
			...
			\addplot[color=black,domain=-2.5:-0.5, smooth] {0};
			...
			\addplot[color=black,domain=0.5:2.5, smooth] {0};
		  \end{axis}
		\end{tikzpicture}
	}
	\caption{Example for a \enquote{cut-off} function $\eta$}
\end{figure}
~\newline		
