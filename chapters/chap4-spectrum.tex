...
where $I_{s} \coloneqq \{ \lambda_{s}(k) : k \in \overline{B} \} ~(s \in \N)$. As $B$ is compact and connected, for each of those sets $I_{s}$ holds that
	...
	\end{equation} 
as $\lambda_{s}(k)$ is a continuous function of $k \in \overline{B}$ for all $s \in \N$, which follows by standard arguments from the fact that the coefficients in the transformed eigenvalue problem \eqref{mod-eigv-problem},  \eqref{periodic-condition} depend continuously on $k$. % (k) in lambda correct?
~\\
Todo: Proof Plum % todo Martin proof
~\\
...
\begin{theorem} \label{4.1:thm-MainResult.FirstInclusion}
	...
	\begin{proof}
		...
		\end{equation}
		We regard $\psi_{s}(\cdot, k)$ as extended to the whole of $\R$ by the boundary condition \eqref{quasi-periodic-condition}, whence, due to the periodicity of $A$, \eqref{firstinclusion-firstequation} holds for all $x \in \R$ and $\psi_{s} \in H^{2}_{loc}(\R)$. \\ % todo Martin nachlesen ggf. Marksus
		...
		\end{equation} % todo Martin/Markus more expicitely?!
