...
\begin{theorem}
	...
	\begin{proof}
		...
		In particular, the boundary value problem\footnote{todo those next 3 lines!} % todo #8: This, I haven't showed yet either...
		\begin{eqnarray}
			(A - \lambda I) v(\cdot, k) & = & (Uf)(\cdot, k) \text{ on } \Omega, \label{3.33} \\
			v(\frac{1}{2}) & = & e^{ik} v(-\frac{1}{2}) \notag
		\end{eqnarray}
		has a unique solution for every $k \in B$. Bloch wave expansion\footnote{check this} gives % todo #9: Is it correct that this simply means the completeness of Bloch waves as Basis of H1(Omega) which is the range of U (*,k)?
		\begin{align*}
		...
		Since $A$ is closed\footnote{That's clear but I will write some stuff down for that}, \eqref{ulflconvergence} now implies % todo #10: A self-adjoint => A closed that's clear but I guess Im gonna show this in the introduction
		...
		are bounded by $C \| (Uf)(\cdot, k) \|^{2}_{L^{2}(\Omega)} \| \psi_{s}(\cdot, k) \|^{2}_{L^{2}(K)}$ the latter factor is bounded as a function of $k$ because $K$ is covered by a finite number of copies of $\Omega$, and the former is in $L^{1}(B)$ by Theorem \ref{3.4.1}.\footnote{Isn't this in L2B?} % todo #12: I have problems understanding why this is in L1
		
		Since $K \times B$ is bounded, $r$ and $t$ are also $L^{1}(K \times B)$. \footnote{again L2?} Therefore, Fubini's Theorem implies that the order of integration with respect to $x$ and $k$ may be exchanged for $r$ and $t$. Thus, by \eqref{ul}, % todo #12: I have problems understanding why this is in L2
			\begin{align*}
			...
			\end{align*}
			Since $\varphi$ has compact support in the interior of $K$, $(A - \lambda I)$ may be moved to $\psi_{s}(\cdot, k)$,\footnote{How is this important?} and hence \eqref{eigv-problem} gives % todo #13: I don't see how the compact support is needed. Convergence?
			\begin{align*}
