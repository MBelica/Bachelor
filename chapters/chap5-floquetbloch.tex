...
In Chapter \ref{chap:6} we will show that the spectrum of the operator $A$ can be constructed from the eigenvalue sequences $(\lambda_{s}(k))_{s \in \N}$ introduced above by varying $k$ over the Brillouin zone $B$. For this purpose we will need two results, involving the Floquet transformation and the Bloch waves, to be able to move the problem from $L^{2}(\R)$ to $L^{2}(\Omega \times B)$. For the sake of completeness, we include here the proofs of both theorems, as given in \cite[Section 3.4, 3.5]{dorfler2011photonic}. As we intend to use these results used in both the one-dimensional case and the multi-dimensional case, expressions like $|B|$ and $|\Omega|$ won't be simplified to be applicable in both chapters. 
...
\begin{theorem} \label{3.2:thm-UIsometricIsomorphism}
	...
	\begin{proof}
		...
		\end{equation} 
		We can apply the Monotone Convergence Theorem which shows that
		...
		forms an orthonormal basis of $L^{2}(B)$ and Parseval's identity gives for these $x$
		\[ \int_{B}|(Uf)(x,k)|^{2} dk = \sum_{n \in \Z} |f(x - n)|^{2}. \]
	 	This expression is in $L^{2}(\Omega)$ by \eqref{functionoverperiodicity} and Fubini's theorem, and we conclude $\| Uf \|_{L^{2}(\Omega \times B)} = \|f\|_{L^{2}(\R)}$. It is for us still left to show that the mapping $U$ is surjective, and that $U^{-1}$ is given by \eqref{3.8} or \eqref{3.9}. For $g \in L^{2}(\Omega \times B)$, let us define
		\begin{equation}
			f(x - n) \coloneqq \frac{1}{\sqrt{|B|}} \int_{B} g(x, k) e^{-ikn} dk \quad (x \in \Omega, n \in\Z).\label{3.11}
		\end{equation}
		Using Fubini's theorem we know that for almost every $x \in \Omega$ we have $g(x, k) \in L^{2}(B)$, and with this, Parseval's identity states for fixed $x \in \Omega$ that $\sum_{n \in \Z} |f(x - n)|^{2} = \int_{B} |g(x, k)|^{2} dk$. Integrating this equality over $\Omega$ and using Fubini's theorem and the Monotone Convergence Theorem then yields
		\begin{align*}
...
\begin{theorem} \label{3.3:thm-flConvergence}
	...
	\begin{proof}
		...
		\[ \lim_{l \rightarrow \infty} \int_{B} \| (Uf)(\cdot, k) - g_{l}(\cdot, k) \|^{2}_{L^{2}(\Omega)} dk  = \int_{B} \lim_{l \rightarrow \infty}  \| (Uf)(\cdot, k) - g_{l}(\cdot, k) \|^{2}_{L^{2}(\Omega)} dk = 0. \]
		  All in all, this means by Fubini's Theorem
			\begin{equation}
