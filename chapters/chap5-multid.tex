...
\begin{figure}[!ht] \centering
	...
	\caption{Periodicity cell with normal $\eta$ on $S$}
\end{figure}

Now, restricting again this problem to a fundamental domain of periodicity, for example $Y$,
% todo todo
We consider, analogous to before, the problem to find $u \in H^{1}_{k, n} \coloneqq \big\{ w \in H^{1}(Y) \colon w \big|_{S_{j}^{+}} = w \big|_{S_{j}^{+}} e^{i k_{j}}$ for $k \in [-\pi, \pi]^{2}, j = 1,2 \big\}$ such that
	\begin{equation}
...
Again, by Lax-Milgram's Theorem the existence of a unique solution $u \in H^{1}_{k, n}$ is ensured if $\mu$ is small enough, and the operator $R_{\mu, k} \colon f \mapsto u$ is in return well-defined and injective. This allows us to define 
