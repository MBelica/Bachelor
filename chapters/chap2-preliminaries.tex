...
Of special interest in this thesis will be one distribution, the Dirac delta function $\delta_{x_{0}}$ where $x_{0} \in \R$. For a given $x_{0} \in \R$ we first define the sequence of functionals
	...
	\end{equation}
 	...
 	\begin{remark}
 		 The Dirac delta function can even be defined equivalently either as a distribution or as a measure.
 	\end{remark}
~\\
...
\begin{definition}
	Let $U_{X} = \{ x \in X \colon |x| < 1 \}$. Let $X$ be a normed space and $Y$ a Banach space. A linear operator $A \colon X \rightarrow Y$ is called compact, if $T(U_{X})$ is relativ compact in $Y$.
\end{definition}
