...
Of special interest in this thesis will be one distribution, the Delta-Distribution $\delta_{x_{0}}$ where $x_{0} \in \R$. For a given $x_{0} \in \R$ we first define the sequence of functionals % todo anders herum definieren
	\begin{equation}
	...
	\end{equation}
 	Every functional is symmetric around $x_{0}$ and its support converges to the point $x_{0}$ as $\epsilon$ tends to $0$. The Delta-Distribution is then defined as
 	\[ \delta_{x_{0}}(f) \coloneqq \lim_{\epsilon \rightarrow 0} \delta_{\epsilon}(f). \]
 	This limit exists and it holds that $\delta_{x_{0}} \in \mathfrak{D}'(\R)$ and $\delta_{x_{0}}(f) = \lim_{\epsilon \rightarrow 0} \delta_{\epsilon}(f) = f(x_{0})$, for a proof see \cite[section 1.4]{weis2015spektheo}.
 	\begin{remark}
...
Moreover, we will study a certain class of operators, the self-adjoint operators. For this we define:
...
Eventually, in chapters \ref{chap:4} and \ref{chap:7} we will examine so called compact operators and some of their properties.
