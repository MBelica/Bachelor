...
In this thesis, we are interested in the spectrum of the operator describing the situation of the Kronig-Penney model when the particle moves through periodically distributed, singular potentials. With respect to the above this means taking the limit $b \rightarrow 0$ while $V_{0}$ remains of order $\rho b^{-1}$. We will show that such an operator has a spectrum consisting of a union of compact intervals in $\R$. 
~\\

The remainder of this thesis is structured as follows. We begin with some preliminaries in chapter 2 to review some concepts from functional analyses and spectral theory. The material presented in chapter 3 is based on the work \cite[section 3.4]{dorfler2011photonic}. We will examine the aforementioned operator and show that its self-adjointness, which is our first step in analysing its spectrum. We proceed in chapter 4 by restricting the problem to its fundamental domain of periodicity. There we are able to analyse the spectrum of the restricted operator by showing the compactness of its resolvent. In chapter 5 we introduce methods to transfer the results from the restricted case into the unrestricted. Based on this methods, we are able to show in chapter 6 the main result, that such an operator has a spectrum consisting of a union of compact intervals in $\R$, for the one-dimensional case and extrapolate this result for the multi-dimensional case in chapter 7. We close this thesis in chapter 8, where we discuss possible gaps between the compact intervals and list current researches.