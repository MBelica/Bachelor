...
\begin{theorem}
	...
	Though such $u_{j}$ has to satisfy
		\[ \int_{\Omega} u_{j}' v' + \rho u_{j}(x_{0}) v(x_{0}) - \mu \int_{\Omega} u_{j} v = \int f_{j} v \quad \forall v \in H^{1}_{k} \]
	Now, choosing here $v = u_{j}$ \eqref{eq:potentialestim} yields for $\mu$ small enough % todo do this!!!!!!!
		\[  \| u_{j} \|_{H^{1}(\Omega)} \leq | \int_{\Omega} f_{j} v | \leq \| f_{j} \|_{L^{2}(\Omega)} \| u_{j} \|_{L^{2}(\Omega)} \leq c \sqrt{vol(\Omega)} \]
	Which shows that this $u_{j}$ is bounded in $H^{1}(\Omega)$. As $H^1(\Omega) \subset C(\Omega)$: 
		...
		\end{equation} 
		From \eqref{eq:H1estimation} for a $f \in B_{H^{1}}$ it follows that 
		\[ |f(x)|^{2} \leq 2 \| f \|^{2}_{L^{2}} + 2 \leq 4 \quad \forall x \in \Omega\]
		And with that we can approximate a $f \in B$ by simple functions through partitioning $\Omega$ into $n_{\epsilon}$ equidistant intervals. As our simple function is constant on each subinterval, we chose this constant $c_{k}$ such that $|f(\frac{k}{n}) - c_{k + 1}| < \frac{1}{n}$ which leads to
		\begin{align*}
		...
		\end{align*}		
		Which means $\forall f \in B_{H^{1}_{k}} \coloneqq \{ f \in H^{1}_{k}(\Omega) : \| f \| \leq 1 \} ~\exists g \in \{ g_{1}, \dotsc, g_{n_{\epsilon}} \} : \quad \| f - g \|\leq \epsilon$. Together with the closure of $H^{1}_{k}$ this yields the compact embedding of $H^{1}_{k}$ in $L^{2}(\Omega)$ such that $R_{\mu, k}$ is compact.
	\end{proof}	
\end{theorem}
